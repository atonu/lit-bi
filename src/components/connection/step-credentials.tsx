          ...
          <div className="flex gap-3">
            {(["MONGODB", "POSTGRESQL", "MYSQL"] as const).map((eng) => (
              <button
                ...
                className={`flex flex-1 items-center justify-center gap-1.5 rounded-lg border px-2 py-2 text-[10px] sm:text-xs font-medium transition-all ${creds.engine === eng
          ...
          <>
            ...
            <div className="flex items-center gap-3 rounded-lg border border-border/40 bg-muted/20 px-4 py-3">
              <button
                ...
                className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${creds.sslEnabled ? "bg-chart-1" : "bg-muted-foreground/30"
                  }`}
              >
                <span
                  className={`pointer-events-none inline-block size-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${creds.sslEnabled ? "translate-x-4" : "translate-x-0"
                    }`}
                />
              </button>
              <div>
